...
public class PlayBackgroundScreen : MonoBehaviour
{
	[SerializeField]
	...
	private void Start ()
	{
		...
		Sprite[] sourceSprite = ImageSettings.Instance.background;
		SpawnPanels (sourceSprite);
		panelControl [GameSettings.Instance.visualPlayBackgroundSet].SetActive (true);
	}
	private void SpawnPanels (Sprite[] spriteList)
	{
		...
		for (int index = 0; index < panelCount; index++)
		{
			...
			obj.transform.SetParent(parentConteinerRT);
	...
	private void GeneralClick (int spriteIndex)
	{
		...
		panelControl [GameSettings.Instance.visualPlayBackgroundSet].SetActive (false);
		panelControl [spriteIndex].SetActive (true);
		GameSettings.Instance.visualPlayBackgroundSet = spriteIndex;
	}
	public void OnBack ()
	{
